...
#include "ext/portable_executable/image.hpp"
...
std::uint8_t sys::kernel::parse_modules()
{
	...
	std::uint64_t current_entry = read_kernel_virtual_memory<std::uint64_t>(start_entry);

	while (current_entry != start_entry)
	{
		kernel_module_t kernel_module = { };

		std::uint64_t module_base_address = read_kernel_virtual_memory<std::uint64_t>(current_entry + 0x30);
		std::uint32_t module_size = read_kernel_virtual_memory<std::uint32_t>(current_entry + 0x40);
		std::string module_name = user::to_string(read_unicode_string(current_entry + 0x58));
	...
	if (sys::kernel::modules_list.contains(ntoskrnl_name))
	{
		...
		else
		{
			...
			else if (sys::mm_physical_memory_block < 0xffff800000000000ULL)
			{
				...
				sys::mm_physical_memory_block = 0;
			}
			else
			{
...
std::uint32_t sys::get_image_size(std::uint64_t base_address)
{
	...
	std::vector<std::uint8_t> nt_headers(256);
...
std::uint64_t sys::find_target_module_base(std::uint32_t target_pid, std::uint64_t target_cr3, const std::string& module_name) {
	...
	while (current_entry != list_head) {
		LDR_DATA_TABLE_ENTRY_MINIMAL table_entry;
...
void* sys::get_ntoskrnl_base()
{
    ...
    {
        sys::kernel_module_t& ntoskrnl = sys::kernel::modules_list[ntoskrnl_name];
        std::println("Found ntoskrnl.exe at: 0x{:x}", ntoskrnl.base_address);
    ...
    {
        std::println("Found ntoskrnl.exe at: 0x{:x}", ntoskrnl_info->base_address);
        return reinterpret_cast<void*>(ntoskrnl_info->base_address);
    }

    std::println("Failed to find ntoskrnl.exe base address");
    return nullptr;
}
...
{
	constexpr std::uint32_t chunk_size = 0x1000;
	...
	for (std::uint32_t offset = 0; offset < size; offset += chunk_size) {
		...
		for (std::uint32_t i = 0; i <= read_size - pattern_size; ++i) {
			...
			if (match) {
				std::println("[+] Pattern found at: 0x{:X}", base_address + offset + i);
...
std::uint64_t sys::get_process_cr3(std::uint32_t pid)
{
	return hypercall::get_process_cr3(pid, sys::ps_initial_system_process, sys::current_cr3);
}
