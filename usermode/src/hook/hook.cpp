...
{
	std::cout << "[DEBUG] Starting hook setup..." << std::endl;
	std::cout << "[DEBUG] kernel_detour_holder_base: 0x" << std::hex << kernel_detour_holder_base << std::dec << std::endl;
	
	if (kernel_detour_holder_base == 0) {
		std::cout("[!] FAILED: kernel_detour_holder_base is 0");
		return 0;
	}
	...
	{
		std::cout("[!] FAILED: Unable to allocate shadow page memory");
		return 0;
	}
	std::cout("[DEBUG] Allocated shadow page at: 0x{:x}", reinterpret_cast<std::uint64_t>(kernel_detour_holder_shadow_page_mapped));
	...
	{
		std::cout("[!] FAILED: Unable to translate shadow page virtual address to physical");
		return 0;
	}
	std::cout("[DEBUG] Shadow page physical address: 0x{:x}", shadow_page_physical);
	...
	{
		std::cout("[!] FAILED: Unable to translate kernel_detour_holder_base to physical address");
		return 0;
	}
	std::cout("[DEBUG] Kernel detour holder physical page: 0x{:x}", kernel_detour_holder_physical_page);

	// in case of a previously wrongfully ended session which would've left the hook still applied
	std::cout("[DEBUG] Removing any existing SLAT code hook...");
	hypercall::remove_slat_code_hook(kernel_detour_holder_physical_page);

	std::cout("[DEBUG] Reading original page content...");
	hypercall::read_guest_physical_memory(kernel_detour_holder_shadow_page_mapped, kernel_detour_holder_physical_page, 0x1000);

	std::cout("[DEBUG] Adding SLAT code hook with Target GPA: 0x{:x}, Shadow GPA: 0x{:x}", kernel_detour_holder_physical_page, shadow_page_physical);
	std::uint64_t hook_status = hypercall::add_slat_code_hook(kernel_detour_holder_physical_page, shadow_page_physical);

	if (hook_status == 0)
	{
		std::cout("[!] WARNING: Unable to add SLAT code hook - continuing without kernel hook support");
		std::cout("[DEBUG] This means kernel-level API hooking will not work, but other features should function");
	...
	std::cout("[DEBUG] SLAT code hook added successfully");

	std::cout("[DEBUG] Setting up kernel detour holder...");
	kernel_detour_holder::set_up(reinterpret_cast<std::uint64_t>(kernel_detour_holder_shadow_page_mapped), 0x1000);

	std::cout("[+] Hook setup completed successfully");
	return 1;
}
...
std::uint8_t hook::remove_kernel_hook(std::uint64_t hooked_routine_virtual, std::uint8_t do_list_erase)
{
	...
	{
		std::cout("unable to find kernel hook");
	...
	{
		std::cout("unable to remove slat counterpart of kernel hook");
	...
	{
		std::cout("unable to remove slat counterpart of kernel hook (2)");
	...
	{
		std::cout("unable to deallocate mapped shadow page");
