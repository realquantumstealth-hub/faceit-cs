...
std::uint64_t hypercall::read_guest_virtual_memory(void* guest_destination_buffer, std::uint64_t guest_source_virtual_address, std::uint64_t source_cr3, std::uint64_t size)
{
	...
	// Retry mechanism for TLB timing issues (both Intel and AMD)
	for (int retry = 0; retry < 3; ++retry) {
		std::uint64_t result = make_hypercall(hypercall_info.call_type, hypercall_info.call_reserved_data, reinterpret_cast<std::uint64_t>(guest_destination_buffer), guest_source_virtual_address, size);
		if (result == size) {
			return result; // Success
		}
		if (retry < 2) {
			// Small delay to allow TLB to update
...
std::uint64_t hypercall::write_guest_virtual_memory(void* guest_source_buffer, std::uint64_t guest_destination_virtual_address, std::uint64_t destination_cr3, std::uint64_t size)
{
	...
	// Retry mechanism for TLB timing issues (both Intel and AMD)
	for (int retry = 0; retry < 3; ++retry) {
		std::uint64_t result = make_hypercall(hypercall_info.call_type, hypercall_info.call_reserved_data, reinterpret_cast<std::uint64_t>(guest_source_buffer), guest_destination_virtual_address, size);
		if (result == size) {
			return result; // Success
		}
		if (retry < 2) {
